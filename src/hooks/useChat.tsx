...
export const useChatHook = (initialHistory: any) => {
    ...
    const askQuestion = async () => {
        ...
        if (data && data.message) {
            const msg = data.message.startsWith("AI: ") ? data.message.substring(4).trim() : data.message.substring(4).trim();
            setResponse(msg);
        }
    };

    const updateJunoStore = async (id: string, question: string, response: string) => {
        TODO:// //replace undefined with actual user's id
        await insertPromptResponse("undefined", question, response);
    };
    ...
    useEffect(() => {
        if (response) {
            ...
            setResponding(false);
            setResponse(null);
            updateJunoStore("", updatedPairs[tempPairs.length - 1].human_message, response);
        }
    }, [response, tempPairs]);
